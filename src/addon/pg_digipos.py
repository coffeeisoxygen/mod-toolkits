...
def login_block():
    ...
    """
    with st.expander("1. Login Akun Digipos", expanded=True):
        with st.form("login_form"):
            st.session_state.username = st.text_input(
                "Username",
                value=st.session_state.username,
                key="digipos_username_input",
            )
...
def otp_block():
    ...
    """
    with st.expander("2. Verifikasi OTP (Jika Diperlukan)", expanded=True):
        with st.form("otp_form"):
            otp = st.text_input("OTP", key="digipos_otp_input")
            submitted = st.form_submit_button("Verifikasi OTP")

        if submitted:
            url = f"{BASEURL}/add_account_otp?username={st.session_state.username}&otp={otp}"
...
def info_block():
    ...
    """
    with st.expander("3. Info Akun (Profile & Balance)", expanded=True):
...
def logout_block():
    ...
    """
    with st.expander("4. Logout Akun Digipos", expanded=True):
